...
#pengerjaan di .ipynb di pindah .py

#Menyiapkan data
#df[['Attack', 'Defense']] = df[['Attack', 'Defense']].astype(float)
# df['Attack'] = df['Attack'].astype(float)
# print(df.info())
# analisa = df.iloc[:, 5:7]
# analisa_array = np.array(analisa)

# #Machine Learning
# kmeans = KMeans(n_clusters = 3, random_state=123)
# kmeans.fit(analisa_array)
# analisa["kluster"] = kmeans.labels_

# #Melakaukan analisa statistika deskriptif
# print(analisa.describe())

# #visualisasi
# fig, ax = plt.subplots()
# sct = ax.scatter(analisa.Attack, analisa.Defense, s = 100, c = analisa.kluster, marker = ".", alpha = 0.5)
